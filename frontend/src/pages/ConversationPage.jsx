...
const ConversationPage = () => {
  ...
  useEffect(() => {
    markConversationRead(conversationId)
      ...
      .catch((error) =>
        logClientError("mark_conversation_read_failed", error, {
          conversationId,
        }),
      );
      ...
      <PageShell>
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <p className="text-xs text-muted">
            Realtime: {realtimeEnabled ? status : "disabled"}
          </p>
        </div>
        ...
        {messagesQuery.isLoading ? (
          ...
          <ErrorState
            ...
            requestId={getApiErrorRequestId(messagesQuery.error)}
              ...
              {messages.length === 0 ? (
                ...
                messages.map((message) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`max-w-[76%] rounded-md px-4 py-2 text-sm ${mine ? "bg-primary text-white" : "bg-surface-soft text-ink"}`}
                      >
                        <p>{message.content}</p>
                        {mine && (
                          <button
